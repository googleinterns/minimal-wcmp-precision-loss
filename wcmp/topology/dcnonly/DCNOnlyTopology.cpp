	...
	for (int i=0; i < numSbPerDcn; ++i)
		for (int j=0; j < numMbPerSb; ++j)
			for (int k=0; k < numS3PerMb; ++k) {
				mainprog::Switch tmp_sw = {i, j, k, mainprog::SwitchType::s3, sw_gid};
	...
	for (int i=0; i < numSbPerDcn; ++i)
		for (int j=0; j < numMbPerSb; ++j)
			for (int k=0; k < numS2PerMb; ++k) {
				mainprog::Switch tmp_sw = {i, j, k, mainprog::SwitchType::s2, sw_gid};
				switches_.push_back(tmp_sw);
				per_sb_switches_[i].push_back(sw_gid);
				++sw_gid;
			}
	...
	for (int i=0; i < numSbPerDcn; ++i)
		for (int j=0; j < numMbPerSb; ++j)
			for (int k=0; k < numS1PerMb; ++k) {
				mainprog::Switch tmp_sw = {i, j, k, mainprog::SwitchType::s1, sw_gid};
				switches_.push_back(tmp_sw);
				per_sb_switches_[i].push_back(sw_gid);
				++sw_gid;
			}
...
void DCNOnlyTopology::AddLinks() {
	int link_gid = 0;
	per_pair_links_ = std::vector<std::vector<std::vector<int>>>(numSbPerDcn, std::vector<std::vector<int>>(numSbPerDcn));
			...
			if (dst_sb != src_sb)
				...
				for (int src_mb=0; src_mb < numMbPerSb; ++src_mb) {
					...
					for (int i=0; i < numS3PerMb; ++i) {
						...
						mainprog::Link tmp_link = {switches_[src].gid, switches_[dst].gid, std::min(test_SBs[src_sb], test_SBs[dst_sb]), link_gid};
...
void DCNOnlyTopology::AddPaths() {
	...
	per_pair_paths_ = std::vector<std::vector<std::vector<int>>>(numSbPerDcn, std::vector<std::vector<int>>(numSbPerDcn));
		...
		for (int dst_sb=0; dst_sb < numSbPerDcn; ++dst_sb)
			if (dst_sb != src_sb) {
				...
				for (int link_gid : direct_paths) {
					...
					per_pair_paths_[src_sb][dst_sb].push_back(path_gid); // add path to per pair paths
					...
					if ((trans_sb != src_sb) && (trans_sb != dst_sb)) {
						...
						for (int first_gid : first_hops) {
							for (int second_gid : second_hops) {
								...
								per_pair_paths_[src_sb][dst_sb].push_back(path_gid);
...
SCIP_RETCODE DCNOnlyTopology::CreateVariableWeight(SCIP* scip, std::vector<std::vector<std::vector<SCIP_VAR*>>> &x) {
	x = std::vector<std::vector<std::vector<SCIP_VAR*>>>(numSbPerDcn, std::vector<std::vector<SCIP_VAR *>>(numSbPerDcn));
	for (int src_sb=0; src_sb < numSbPerDcn; ++src_sb) {
		for (int dst_sb=0; dst_sb < numSbPerDcn; ++dst_sb) {
...
SCIP_RETCODE DCNOnlyTopology::CreateConstraintsLinkUtilizationBound(
	...
	for (int i=0; i<links_.size(); ++i) {
		...
		for (int path_gid : per_link_paths_[i]) {
			...
			values.push_back(traffic_matrix_[src_sb * numSbPerDcn + dst_sb] / double(links_[i].capacity));
		}
...
SCIP_RETCODE DCNOnlyTopology::find_best_dcn_routing() {
	...
	scip_result_ = std::vector<std::vector<std::vector<double>>>(numSbPerDcn, std::vector<std::vector<double>>(numSbPerDcn));
	...
	for (int src_sb=0; src_sb < numSbPerDcn; ++src_sb)
		for (int dst_sb=0; dst_sb < numSbPerDcn; ++dst_sb) {
			...
			for (int p : per_pair_paths_[src_sb][dst_sb]) {
				std::cout << SCIPgetSolVal(scip, sol, x[src_sb][dst_sb][paths_[p].per_pair_id]) << ", ";
				scip_result_[src_sb][dst_sb].push_back(SCIPgetSolVal(scip, sol, x[src_sb][dst_sb][paths_[p].per_pair_id]));
			}
			std::cout << std::endl;
		}
	...
	for (int src_sb=0; src_sb < numSbPerDcn; ++src_sb)
		for (int dst_sb=0; dst_sb < numSbPerDcn; ++dst_sb)
			if (src_sb != dst_sb) {
				...
				for (int p : per_pair_paths_[src_sb][dst_sb]) {
					double traffic_amount = traffic_matrix_[src_sb * numSbPerDcn + dst_sb] * scip_result_[src_sb][dst_sb][paths_[p].per_pair_id];
					if (traffic_amount > 0) {
						std::cout << traffic_amount << " Gbps of demand from u" << src_sb << " -> u" << dst_sb
						<< " is placed on DCN link " << links_[paths_[p].link_gid_list.front()].gid << std::endl;
					}
				}
