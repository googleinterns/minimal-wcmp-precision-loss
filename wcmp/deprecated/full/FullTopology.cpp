...
std::vector<Path> FullTopology::FindDCNPaths(int src_sw, int dst_sw) {
  ...
  for (int link_gid : direct_paths) {
    Path tmp_path = {{link_gid},
                     ...
                     -1, -1};
    paths.push_back(tmp_path); // add path to the path list
  }
...
//          per_sb_pair_paths_[src_idx][dst_idx].push_back(
//            path_gid);
...
// add all the paths
void FullTopology::AddPaths(int scope) {
  int path_gid = 0;
  // initialize the path vectors
  per_st_pair_paths_ = std::vector<std::vector<std::vector<int>>>(src_sw_group.size(),
  ...
  for (int src_idx = 0; src_idx < src_sw_group.size(); ++src_idx)
    for (int dst_idx = 0; dst_idx < dst_sw_group.size(); ++dst_idx)
      if (src_sw_group[src_idx] != dst_sw_group[dst_idx]) {
        int index = 0;
        if (scope == 0) {
          // add the direct paths
          std::vector<int> direct_paths;
...
// set the problem scope to AbstractTopology level, or S2-S2 level, or intra-domain level
void FullTopology::SetProblemScope(int scope) {
  ...
  // add paths for each source destination pair
  AddPaths(wcmp::problem_scope);
}
