...
// The AbstractTopology network with only the top level, namely, AbstractTopology level
...
private:
  ...
  std::vector<std::vector<std::vector<int>>> per_st_pair_paths_;
  ...
  std::vector<Path> FindIntraDomainPaths(int src_sw, int dst_sw);
